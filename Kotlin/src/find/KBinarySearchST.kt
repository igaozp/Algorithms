 ...
 * @version 1.0
 */
class KBinarySearchST<K : Comparable<K>, V> constructor(capacity: Int) {
    // 默认的初始化大小
    private val INIT_CAPACITY = 2
    // 存储键的数组
    private var keys: MutableList<K?> = MutableList(INIT_CAPACITY, { null })
    // 存储值的数组
    private var vals: MutableList<V?> = MutableList(INIT_CAPACITY, { null })
    // 查找表的长度
    private var size = 0
...
fun main(args: Array<String>) {
    val st = KBinarySearchST<String, Int>(2)
